...
{
    public partial class BrowserSelection : Form
    {
        public BrowserSelection()
        {
            ...
            Load += (async (sender, args) =>
                        ...
                        test:
                            try
                            {
                                List<string> items = await Online.ServerCom.ReadTextFromFile("https://sethdiscordbot.000webhostapp.com/Storage/Installers/BrowserList");
                                ...
                                checkedListBox1.ItemCheck += (sender2, e2) =>
                                {
                                    if (e2.NewValue == CheckState.Checked)
                                        Globals.SelectedBrowsers.Add(items[e2.Index].Replace("\n", "").Replace("\r", ""));
                                    else if (e2.NewValue == CheckState.Unchecked) Globals.SelectedBrowsers.Remove(items[e2.Index].Replace("\n", "").Replace("\r", ""));
                                };
                            }
                            catch
                            {
